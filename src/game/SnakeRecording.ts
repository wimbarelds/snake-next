...
export class SnakeRecording {
  ...
  private tick() {
    const input: Direction | null =
      this.inputHistory.UP.indexOf(this.tickIndex) >= 0
        ? 'UP'
        : this.inputHistory.DOWN.indexOf(this.tickIndex) >= 0
        ? 'DOWN'
        : this.inputHistory.LEFT.indexOf(this.tickIndex) >= 0
        ? 'LEFT'
        : this.inputHistory.RIGHT.indexOf(this.tickIndex) >= 0
        ? 'RIGHT'
        : null;
