...
export class SnakeGame {
    ...
    constructor(playId: string, level: Level, numPlayers: number) {
        ...
        this.agents = new Array(numPlayers).fill(0).map((x, agentIndex) => new SnakeAgent(this, this.level.snakeTiles[agentIndex]));
        this.wallTiles = this.level.wallTiles.slice().sort((a, b) => ((a.x === b.x) ? (a.y - b.y) : (a.x - b.x))); // if we sort our wall tiles we search for collision checks much faster
