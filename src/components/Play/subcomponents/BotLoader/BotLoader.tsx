...
export function BotLoader({numPlayers, onClose, onPlayBot}: Props) {
  ...
  const onFileSubmit = useCallback((e: Event) => {
    e.preventDefault();
    if (!file) return alert('Must pick file');
    const bots: Worker[] = new Array(numPlayers).fill(null).map(() => new Worker(
      URL.createObjectURL(file)
    ));
    onPlayBot(bots);
  }, []);

  const onURLSubmit = useCallback(async (e: Event) => {
    e.preventDefault();
    ...
    <div className="overlay__content modal">
        ...
        <div className="modal__body">
            ...
            <form className={styles.botLoaderOption} onSubmit={onURLSubmit}>
                ...
                <div className="form">
                    <input type="url" placeholder="http://" value={botUrl} onInput={e => setBotUrl((e.target as HTMLInputElement).value)} />
