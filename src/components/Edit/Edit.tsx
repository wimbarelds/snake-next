...
const normalizedCursorPosFromEvent = (e: React.MouseEvent<HTMLCanvasElement, MouseEvent>, canvas: HTMLElement): Pos => {
  return {
...
export function Edit() {
  ...
  useEffect(() => {
    ...
    if (curSnakeIndex >= numSnakes) {
      ...
      // Add a snake.
      setSnakeTiles(snakeTiles => [
        ...snakeTiles,
        newSnake,
      ]);
      // Remove walls in places where the new Snake is being placed.
      setWallTiles(wallTiles => wallTiles.filter(wallPos => !newSnake.some(snakeDot => snakeDot.x === wallPos.x && snakeDot.y === wallPos.y)));
    }
  }, [curSnakeIndex, numSnakes]);

  const drawWallTile = useCallback((x: number, y: number) => {
  ...
  const eraseWallTile = useCallback((x: number, y: number) => {
    const context = wallCanvasRef.current?.getContext('2d');
    if (!context) return;
  ...
  const addWallTile = useCallback((pos: Pos) => {
  ...
  const placeSnake = useCallback((pos: Pos, snakeIndex?: number) => {
    ...
    if (newSnake.some(dot => wallTiles.some(tile => dot.x === tile.x && dot.y === tile.y))) {
      setWallTiles(old => old.filter(tile => !newSnake.some(dot => dot.x === tile.x && dot.y === tile.y)));
    }
  }, [wallTiles, curSnakeIndex]);

  const removeWall = useCallback((pos: Pos) => {
  ...
  const onCursorDown = useCallback((event: React.MouseEvent<HTMLCanvasElement, MouseEvent>): void => {
    if (!canvas) return;
  ...
  const mouseMoveHandler = useCallback((event: React.MouseEvent<HTMLCanvasElement, MouseEvent>): void => {
    event.preventDefault();
    if (mouseButtonDownRef.current) onCursorDown(event);
  }, [onCursorDown, mouseButtonDownRef]);

  const mouseDownHandler = useCallback((event: React.MouseEvent<HTMLCanvasElement, MouseEvent>): void => {
    event.preventDefault();
  ...
  const save = useCallback(async () => {
    ...
    const result = await saveMap({levelName: name, sessionId: sessionIdRef.current, wallTiles, snakeTiles});
    if (result.success) {
  ...
  const reset = useCallback(() => {
    ...
    setWallTiles([]);
    setCurSnakeIndex(0);
    (wallCanvasRef.current?.getContext('2d') as CanvasRenderingContext2D)?.clearRect(0, 0, canvasWidth, canvasHeight);
  }, []);
    ...
    <main className={styles.edit}>
      <div className={styles.menu}>
        <div className={styles.metaInfo}>
          <input type="text" value={levelName} onInput={(e) => setLevelName((e.target as HTMLInputElement).value)} placeholder="level name" />
          <button type="button" onClick={save}>Save</button>
        </div>
        <div className={styles.tools}>
          {tools.map(tool => (
            <button type="button" className={tool === activeTool ? styles.active : ''} key={tool} onClick={() => setActiveTool(tool)}>{ tool }</button>
          ))}
      ...
      <canvas ref={setCanvas} className={styles.canvas} width="1280" height="640" onMouseMove={mouseMoveHandler} onMouseDown={mouseDownHandler} />
    </main>
  )
}